...
//            .map { return "\($0)" } // String으로 변환을 해주고 싶었던것
//            .flatMap {
////                NetworkManager.shared.callBoxOffice(date: $0).debug("Movie") // -> Movie를 리턴하는 커스텀옵져버블 이다 ! func callBoxOffice(date: String) -> Observable<Movie>
////                    .catch { error in
////                        
...
//                //                    }
//                NetworkManager.shared.callBoxOfficeWithSingle(date: $0).debug("Single Movie")
//                    .catch { error in
//                        print("Movie error", error)
//                        let data =  Movie(boxOfficeResult: BoxOfficeResult(dailyBoxOfficeList: []))
